...
const hideScrollbar = async (page) => {
  await page.evaluate(() => {
    if (document.getElementById('uigraph-hide-scrollbar')) return;
    const style = document.createElement('style');
    style.id = 'uigraph-hide-scrollbar';
    style.textContent = `
      * {
...
const showScrollbar = async (page) => {
  await page.evaluate(() => {
    const style = document.getElementById('uigraph-hide-scrollbar');
    if (style) style.remove();
  });
};

const captureByStitching = async (page, options, progressCallback = null) => {
  try {
    await hideScrollbar(page);
    await forceDocumentScroll(page);

    if (progressCallback) {
      await progressCallback('⬆️ Scrolling to top...');
    }
    ...
    for (let i = 0; i < numSections; i++) {
      try {
        ...
        if (progressCallback) {
          await progressCallback(`📸 Capturing section ${i + 1}/${numSections}...`);
        }
      } catch (err) {
    ...
    if (progressCallback) {
      await progressCallback('🔧 Đang ghép ảnh...');
    }
...
const internalCaptureWebsiteCore = async (options, page, progressCallback = null) => {
  options = {
    width: 1920,
    height: 1080,
    scaleFactor: 1,
  ...
  try {
    ...
    if (options.fullPage) {
      ...
      if (hasCustomScroll.found) {
        return await captureByStitching(page, options, progressCallback);
      }
...
captureWebsite.file = async (page, filePath, options = {}, progressCallback = null) => {
  const screenshot = await internalCaptureWebsiteCore(options, page, progressCallback);
  await fs.writeFile(filePath, screenshot, {
    flag: options.overwrite ? "w" : "wx",
  });
};

captureWebsite.buffer = async (page, options, progressCallback = null) =>
  internalCaptureWebsiteCore(options, page, progressCallback);

captureWebsite.base64 = async (page, options, progressCallback = null) => {
  const screenshot = await internalCaptureWebsiteCore(options, page, progressCallback);
